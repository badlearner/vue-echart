<template>
  <div class='search-list'>
    <div class='input-div industry-type'><!--<span class='span-block'>企业状态：</span>-->
      <el-select size='small' v-model='param.data.marketMainType' placeholder='企业状态'
                 @change='industryTypeChange(marketMainType)'>
        <el-option
          v-for='item in marketMainTypeOptions'
          :key='item.value'
          :label='item.label'
          :value='item.value'>
        </el-option>
      ...
      <div class='choose-industry-container' v-show='showIndustry'
        ...
        <ul>
          <li @click.stop='onSelectIndustry(item)' v-for='item in subdata.industryData'
              class='choose-insudtry-item' :class='{active: item.selected}' v-bind:key="item.HYDM">
            <span>{{ item.HYMC }}</span>
          </li>
        </ul>
      </div>
...
<script>
  ...
  export default {
    ...
    computed: {
      currentYear: {
        get () {
          return this.subdata.data.currentYear
        },
        set (value) {
    ...
    methods: {
      ...
      industryTypeChange (type) {
        ...
        var param = {
          ...
          data: {
            /** 1 在营，0 注销 */
            marketMainType: this.param.data.marketMainType
          }
        }
