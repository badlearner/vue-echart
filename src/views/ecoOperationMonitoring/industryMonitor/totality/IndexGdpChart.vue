<template>
    <chart :options="chartOption" name="gdp" ref="indexGdp" @click="chartClick" auto-resize></chart>
</template>
<script>
	...
	export default {
		...
		methods: {
			...
			setChartOption: function (data) {
				this.$refs.indexGdp.mergeOptions({
          title: {
            text: '各行业企业产值图',
            top: 'bottom',
            left: 'center',
          ...
          yAxis: {
					...
					tooltip: {
						formatter: function (params) {
							...
							params.forEach((item) => {
								strArr.push('<span class="chart-dot" style="background-color: ' + item.color + '"></span>' + item.seriesName + '产值 : ' + formatAmount(item.data) + data.unit);
							});
