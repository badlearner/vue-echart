<template>
  <div style="height: 100%;">
    ...
    <div class="hot-map-page-right">
      <div class="hot-map-page-right-top">
        <router-link class="back-to-bar po-z-index" :to="{path: '/MainIndex/ecoOperationMonitoring/zoneMonitor/professionMarket'}"><img :src="chartLinkImage" /></router-link>
        <zone-chart ref="zone" @click="zoneChartClick" @goToChart="goToChart"/>
      </div>
      <div class="hot-map-page-right-bottom">
        <div>
          <el-input placeholder="市场搜索" style="width:160px;margin-right: 5px;margin-left: 5px;" v-model="info.text" size="mini" clearable/>
...
<script>
  ...
  import zoneChart from '@/components/moreZoneDataChart.vue';
  ...
  import ZoneMonitorService from '@/services/ecoOperationMonitoring/industryMonitor';
  export default {
    data() {
      return {
        ...
        columnData3:[
          ...
          {prop: 'professionalNum',label: '企业户数', width:100},
          {prop: 'totalTax',label: '交易（亿元）', width:150},
          {prop: 'quankuTax',label: '全库税收（亿元）', width:170},
          {prop: 'qukuTax',label: '区库税收（亿元）', width:170}
        ],
        tableData3: [{
          number: 1,
          streetName: '琶洲街',
        ...
        columnData2:[
          {prop: 'number',label: '排名', width:50},
          {prop: 'streetName',label: '街镇名称', width:90},
          {prop: 'professionName',label: '企业名称', width:170},
          {prop: 'GDP',label: '产值（亿元）', width:110},
          {prop: 'Tax',label: '税收（亿元）', width:110}
        ],
        tableData2: [{
          number: 1,
          streetName: '官洲街',
          ...
          number: 4,
          streetName: '瑞宝街',
          professionName: '广宇国际商贸城',
          GDP: 500,
          Tax: 200
        }, {
          number: 5,
          streetName: '华洲街',
          professionName: '新百佳小商品城',
          GDP: 500,
          Tax: 200
        }, {
          number: 6,
    ...
    methods: {
      ...
      _setProfessionalMapData() {
        ...
        ZoneMonitorService.getBuildingMapData().then(result => {
          this.$refs.zone.setChartOption(result,'建筑业');
        });
      },
