<template>
  <div class="pos-reative">
    <router-link :to="{path: '/MainIndex/ecoOperationMonitoring/industryMonitor/industryMap'}"><img class="pull-right" v-bind:src="icon" /></router-link>
  ...
  export default{
    ...
    methods: {
      ...
      _setChartsOption(param) {
        service.getIndustryData().then((response) => {
          let item = response;
