<template>
  <div style="height: 100%;">
    ...
    <div class="hot-map-page-right">
      <div class="hot-map-page-right-top">
        <router-link class="back-to-bar po-z-index" :to="{path: '/MainIndex/ecoOperationMonitoring/industryMonitor/realEstate'}"><img :src="chartLinkImage" /></router-link>
  ...
  export default {
    data() {
      return {
        ...
        columnData2:[
          {prop: 'number',label: '排名', width:50},
          {prop: 'streetName',label: '所属街镇', width:90},
          {prop: 'professionName',label: '企业名称', width:170},
          {prop: 'GDP',label: '产值（亿元）', width:105},
          {prop: 'Tax',label: '税收（亿元）', width:105}
        ],
        tableData2: [{
          number: 1,
          streetName: '江海街',
    ...
    methods: {
      ...
      _setProfessionalMapData() {
        ...
        ZoneMonitorService.getRealEstateMapData().then(result => {
          this.$refs.zone.setChartOption(result,'房地产业');
        });
      },
